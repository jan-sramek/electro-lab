...
export const EN_FALLBACK: Record<string, string> = {
  ...
  'lab.modelNote.battery':
    'Ideal DC voltage source with optional series ESR. Keep polarity clear — shorting p to n is flagged as an error.',
  'lab.modelNote.ac_source':
    'Phasor AC source for AC analysis (mag/phase). Set Frequency for transient sine drive; 0 Hz means DC-only in transient.',
  'lab.modelNote.led':
    'Teaching LED: conducts when forward ≥ vf. Current lights it; sustained current above ~25 mA burns it open — always use a series resistor.',
  'lab.modelNote.switch':
    'SPST teaching switch. Closed/open can also follow Open at / Close at times in transient analysis.',
  'lab.modelNote.inductor':
    'Teaching inductor. Opening a current-carrying loop can create a voltage spike — use a flyback diode on inductive loads.',
  'lab.modelNote.potentiometer':
    'Three-terminal divider (or rheostat with two pins). Wiper position 0→1 slides from end a toward end b.',
  'lab.modelNote.pulse_source':
    'Timed pulse for transient labs: starts at V1, jumps to V2 after Delay for Pulse width (optional Period for repeats).',
  'lab.modelNote.voltmeter':
    'Ideal voltage probe (schematic-only — not in the netlist). Place across the node you want to read; it draws no current.',
