...
/// <summary>
/// Teaching piezo/buzzer: on when Va−Vc ≥ vf (same piecewise settle as LED).
/// BoolParams["burned"] → permanently open.
/// </summary>
public sealed class BuzzerModel : IDeviceModel
{
    public string ModelKey => "buzzer";
    ...
    public IReadOnlyList<string> Validate(ElementInstance element)
    {
        ...
        if (!element.Pins.ContainsKey("a") || !element.Pins.ContainsKey("c"))
            errors.Add($"{element.Id}: buzzer requires pins a, c.");
        if (!element.Params.TryGetValue("vf", out var vf) || vf < 0)
            errors.Add($"{element.Id}: buzzer requires params.vf >= 0.");
        if (!element.Params.TryGetValue("ron", out var ron) || ron <= 0)
            errors.Add($"{element.Id}: buzzer requires params.ron > 0.");
        return errors;
    }
